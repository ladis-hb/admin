...
const admin_get_info_list = async (ctx, u) => {
    ...
    switch (index) {
        case 'usersList':
            let users = await ctx.db.collection(config.DB_user_users)
                .find()
        ...
        case "io":
        case "th":
            let devs = await ctx.db.collection(index)
                .find()
...
const modify_user_info = async (ctx, u) => {
    ...
    let result = await await ctx.db.collection(config.DB_user_users)
